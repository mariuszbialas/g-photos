...
function showBigPhoto(x) {
    ...
    const showLeftArrow = checkPhotosNum1(x); // der linke Pfeil verstecken, wann erstes Bils angezeigt ist

    const checkPhotosNum2 = x => {
        const length = photos.length;
        return x === length - 1; 
    }
    const showRigthArrow = checkPhotosNum2(x); // der rechte Pfeil verstecken, wann letztes Bils angezeigt ist
